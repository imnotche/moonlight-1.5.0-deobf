...
public class SubSlider extends BaseButton
{
    ...
    public SubSlider(final Button parent, final Setting<Number> option) {
    ...
    public void draw(final DrawContext context, final int mouseX, final int mouseY) {
        ...
        RenderUtil.rect(context.getMatrices(), (float)this.getX(), (float)this.getY(), (float)(this.currentWidth + this.getX()), (float)(this.getHeight() + this.getY()), ColorUtil.toARGB(NewGui.getInstance().red.getValue(), NewGui.getInstance().green.getValue(), NewGui.getInstance().blue.getValue(), 255));
    ...
    protected void setWidthFromValue() {
        ...
        this.currentWidth = (int)RenderUtil.reCheckSliderRange(val * 93.0f, 0.0f, 93.0f);
    }
    ...
    protected void updateValueFromWidth() {
        ...
        final float reCheckSliderRange = RenderUtil.reCheckSliderRange(val, this.getMin(), this.getMax());
        this.value = reCheckSliderRange;
        val = reCheckSliderRange;
        final Double roundedValue = RenderUtil.roundSliderForConfig(val);
    ...
    public float getMax() {
        final Number inc = this.option.getMax();
        return (inc == null) ? 100.0f : inc.floatValue();
    }
    
    public float getMin() {
        final Number inc = this.option.getMin();
        return (inc == null) ? 0.0f : inc.floatValue();
    }
    
    public float getStep() {
        final Number inc = this.option.getInc();
